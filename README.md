...
- [`ts/`](ts/README.md) — TypeScript / Node. One shared engine (deck, hand evaluation, betting, side pots)
  ...
  local hot-seat play, vs-AI, an online WebSocket server + terminal client, **and** a static web client
  deployable to **GitHub Pages** (`.github/workflows/deploy.yml` at this repo's root builds and publishes it
  automatically). Hex export/import from the CLI or the browser.
...
# Web app, no server needed for local/AI play:
cd ts && npm install && npm run build:web && open public/index.html
```
...
| Online multiplayer | ✅ WebSocket server + terminal/web clients | ✅ TCP server + terminal clients |
...
3. Push to `main` (or trigger `.github/workflows/deploy.yml` manually) — it runs the TS test suite, builds
   `ts/public/engine.bundle.js` with esbuild, and publishes `ts/public`.

Local Pass & Play and Vs AI work standalone on the deployed page — no server required. Online multiplayer
needs a separately hosted WebSocket server (see [`ts/README.md`](ts/README.md#deploying-the-web-client-to-github-pages))
since GitHub Pages can't run one itself.
